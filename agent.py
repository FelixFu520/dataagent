...
def sql_inter(sql_query: str) -> str:
    ...
    并且当前函数是使用sqlite3连接SQLite数据库。
    本函数只负责运行SQL代码并进行数据查询,若要进行数据提取,则使用另一个extract_data函数。
    :param sql_query: 字符串形式的SQL查询语句,用于执行对SQLite数据库中各张表进行查询,并获得各表中的各类相关信息
    :return:sql_query在SQLite数据库中的运行结果。   
    """
...
def extract_data(sql_query: str, df_name: str) -> str:
    """
    用于在SQLite数据库中提取一张表到当前Python环境中,注意,本函数只负责数据表的提取,
    并不负责数据查询,若需要在SQLite中进行数据查询,请使用sql_inter函数。
    同时需要注意,编写外部函数的参数消息时,必须是满足json格式的字符串,
    :param sql_query: 字符串形式的SQL查询语句,用于提取SQLite中的某张表。
    :param df_name: 将SQLite数据库中提取的表格进行本地保存时的变量名,以字符串形式表示。
    :return:表格读取和保存结果
    """
